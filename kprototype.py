...
class KPrototype:
            ...
            for i in range(self.max_iter):
                ...
                for j in range(self.n_clusters):
                    if len(np.where(assignments == j)[0]) == 0:
                        ...
                        center1[j], center2[j] = self.get_center(X1[np.where(assignments == j)], X2[np.where(assignments == j)])
                #print(center1, center2, assignments)
                for j in tqdm(range(X1.shape[0])):
                    dist = 1e30
                    for k in range(self.n_clusters):
                        cur = self.distance(X1[j], center1[k], X2[j], center2[k])
                        #print(j, cur)
                        if cur < dist:
                            dist = cur
                            cur_assignments[j] = k
                ...
                cur_inertia = 0
                for i in range(X1.shape[0]):
                    cur_inertia += self.distance(X1[i], center1[assignments[i]], X2[i], center2[assignments[i]])
    ...
    def get_center(self, points1, points2):
        ...
        for i in range(points2.shape[1]):
            ...
            c2[i] = np.argmax(np.bincount(points2[:, i]))
        #print("getcenter", points1, points2, c1, c2)
        return c1, c2

    def compactness(self, x1, x2, center1, center2, assignments):
        ans = 0
        for i in range(len(center1)):
            cur = 0
            idxs = np.where(assignments == i)[0]
            for idx in idxs:
                cur = cur + self.distance(x1[idx], center1[i], x2[idx], center2[i])
            ans += cur / len(idxs)
        return ans / len(center1) 

    def seperation(self, center1, center2):
        ans, cnt = 0, 0
        for i in range(len(center1)):
            for j in range(i + 1, len(center1)):
                cnt += 1
                ans += self.distance(center1[i], center1[j], center2[i], center2[j])
        return ans / cnt
...
if __name__ == "__main__":
    ...
    model = KPrototype(n_clusters=3, n_inits=1, max_iter=10)
    model.fit(X1, X2)
    #print("Hopkins statistic=", hopkins(np.concatenate((X1, X2), axis=1), X1.shape[0]))
    center1, center2 = model.cluster_centers_
    print("Compactness=", model.compactness(X1, X2, center1, center2, model.labels_))
    print("Seperation=", model.seperation(center1, center2))
    np.save("./cluster.npy", model.labels_)
